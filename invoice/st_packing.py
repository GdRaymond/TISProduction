...
def parse_packing_list(cell_list=[],file='test.xlsx',by_name="RM500BT(TIS16-SO3466)"):
    ...
    for rownum in range(nrows):
        ...
        if str(current_row[0]).strip().upper()=='CTN NO.' and ('G.W' in [str(item).strip().upper() for item in current_row]) or \
           str(current_row[col_colour_detail]).strip().upper()=='SIZE' and str(current_row[0]).strip()=='': #in the case of CNT NO. omited
            row_size_head=rownum
            before_qty=True
            for colnum in range(ncols):
                current_cell=current_row[colnum]
                logger.debug('--current_cell, colnum[%s]=%s'%(colnum,current_cell))
                if str(current_cell).upper()=="CTN NO.":
                    col_from=colnum
                elif str(current_cell).strip().upper()=="NUMBER OF CARTONS":
                    col_carton_qty=colnum
                elif str(current_cell).upper()=="COLOUR" and str(cell_list[rownum-1][colnum]).strip().upper()=='SIZE':
                    col_colour_detail=colnum
                #below find the 1st size head to get the size_list
                elif colnum>col_colour_detail and before_qty and (str(current_cell).strip().upper() not in ["","QTY"]):
                ...
                elif str(current_cell).upper()=="QTY":
                    col_subtotal=colnum
                    before_qty=False
                ...
                elif str(current_cell).upper()=="CARTON MEASURE(CM)":
                    col_length=colnum
                    col_width=colnum+1
                    col_height=colnum+2
    ...
    for rownum in range(nrows):
      ...
      #  print("==Current row No."+str(rownum)+" : "+str(current_row))
        ...
        if str_contain(str(current_row[0]).upper(),['CONCLUSION','COLOUR'],all_any='any'):
            detail_seg=False
            summary_seg=True
        ...
        if detail_seg:
            ...
            if str(current_row[0]).strip().upper()=="CTN NO." or str(current_row[col_colour_detail]).strip().upper()=='SIZE' : #in the case of CNT NO. omited:
                size_list=[]
                ...
                detail_seg=False
                packing_list["total_volume"]=current_row[col_length]
            ...
            #handle the detail carton
                    ...
                    #logger.error("!!!!!!!from_to=%s"%from_to)
                    carton_info['from']=float(from_to[0])
                    if len(from_to)==2: #normal more than 1 carton
                        carton_info['to']=float(from_to[1])
                    elif len(from_to)==1: #only 1 carton
                        carton_info['to']=float(from_to[0])
                    else: #sometimes, it is 1--2
                        for index in range(1,len(from_to)):
                            if from_to[index]!='':
                                carton_info['to']=float(from_to[index])
                                break
                    ...
                    carton_info['per_carton_pcs']=0 # as there is no this info in Smartex document, all set to zero
        ...
        if summary_seg:
            ...
            if str_contain(str(current_row[0]).upper(),['CONCLUSION','COLOUR'],all_any='any'):
                ...
                for colnum in range(ncols):
                    ...
                    if str(current_cell).strip().upper()=="QTY":
                        col_summary_total=colnum
                    if colnum>col_summary_size and str(current_cell).strip().upper()not in ["QTY",""]:
                        size_info={}
                        if isinstance(current_cell,float) and float(int(current_cell))==current_cell:
                            size_info["size_name"]=str(int(current_cell)).strip() #for the size like 72.0,16.0,20.0,convert to 72,16,20
                        else:
            ...
            elif 'TOTAL:' in [str(cell).strip().upper() for cell in current_row] :
                summary_seg=False
        ...
        for colnum in range(ncols):
            current_cell=current_row[colnum]
            if str_contain(current_cell,['TOTAL:']):
                for col in range(colnum+1,ncols-1):
                    if str(current_row[col]).strip()!="":
                        total_combine=current_row[col].split('=')                        
                        packing_list["total_carton"]=float(re.match(r'\d+',total_combine[0].strip()).group())
                        packing_list["total_quantity"]=float(re.match(r'\d+',total_combine[1].strip()).group())
                        break
                        
            elif str(current_cell).strip().upper().startswith("G.W.:"):
            ...
            elif str(current_cell).strip()=="NO:":
                for col in range(colnum+1,ncols-1):
                    if str(current_row[col]).strip()!="":
                        packing_list["invoice_no"]=str(current_row[col]).strip()
                        break
                    
            elif str(current_cell).strip()=="DATE:":
                for col in range(colnum+1,ncols-1):
                    if str(current_row[col]).strip()!="":
...
"""
def validate_packinglist_by_sheet(cell_list=[],filename='',sheetname=''):
